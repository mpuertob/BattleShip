...
export class Barco {
  ...
  private crearBarco(inicial: Coordenada, direccion: Sentido) {
    for (let index = 0; index < this.size; index++) {
      this.posiciones.push(
        new Casilla(new Coordenada(inicial.x, inicial.y), Estado.indemne)
      );
  ...
  public compruebaSolapamiento(casilla: Casilla[]): boolean {
    for (let index = 0; index < casilla.length; index++) {
      for (
        let indexObject = 0;
        indexObject < this.posiciones.length;
        indexObject++
      ) {
        if (
          this.posiciones[indexObject].coordenada.equals(
            casilla[index].coordenada
          )
        ) {
          return true;
        }
